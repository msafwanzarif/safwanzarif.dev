<template>
  <nav id="navBar"
    class="fixed top-0 right-0 left-0 lg:left-96 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md z-30 border-b border-gray-200 dark:border-gray-800">
    <div class="px-6 py-4">
      <div class="flex items-center justify-between">
        ...
        <div class="lg:hidden" @click="scrollToSection('profile')">
    ...
    <div v-show="mobileMenuOpen"
      ...
      <div class="px-6 py-4 space-y-2">
        <a @click="scrollToSection('profile'); mobileMenuOpen = false"
          :class="{ 'text-dev-accent': activeSection === 'profile' }"
          class="block py-2 text-sm font-medium hover:text-dev-accent transition-colors cursor-pointer">
          My Profile
        </a>
        <a v-for="section in sections" :key="section.id" @click="scrollToSection(section.id); mobileMenuOpen = false"
          class="block py-2 text-sm font-medium hover:text-dev-accent transition-colors cursor-pointer"
          :class="{ 'text-dev-accent': activeSection === section.id }">
          {{ section.name }}
        </a>
...
function updateActiveSection() {
  ...
  if (element) {
    ...
    if (scrollPosition >= offsetTop && scrollPosition < offsetTop + offsetHeight) {
      return activeSection.value = "profile"
    }
  }
