<template>
  ...
  <div v-show="showMobileProfile" class="fixed inset-0 bg-black/50 z-50 lg:hidden" @click="closeMobileProfile">
    <div
      class="fixed left-0 top-0 h-full w-full bg-white dark:bg-gray-900 shadow-xl transform transition-transform duration-300 overflow-y-scroll"
      :class="{ 'translate-x-0': showMobileProfile, '-translate-x-full': !showMobileProfile }" @click.stop>
      <div class="flex flex-col h-full items-center justify-center text-center py-12">
        ...
        <div class="px-8">
          ...
          <div class="flex items-center justify-center space-x-3">
            <a v-for="social in config.socialLinks" :key="social.platform" :href="social.url" target="_blank"
              ...
              class="w-12 h-12 rounded-full bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-500  flex items-center justify-center text-dev-accent hover:bg-dev-accent hover:text-white transition-colors">
